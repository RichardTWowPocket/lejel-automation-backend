...
export class AssemblyAIService {
  ...
  private async uploadAudio(audioPath: string): Promise<string> {
    ...
    try {
      const response = await this.withRetry('Upload audio', async () => {
        ...
        return axios.post(`${this.baseUrl}/upload`, fileStream, {
          headers: {
  ...
  private async submitTranscription(
    ...
    this.logger.log(
      `[AssemblyAI] Submitting transcription request (language: ${useAutoLanguage ? 'auto-detect' : languageCode})`,
    );
    ...
    try {
      ...
      const response = await this.withRetry('Submit transcription', async () =>
        axios.post(`${this.baseUrl}/transcript`, body, {
          headers: {
            Authorization: this.apiKey,
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        const response = await this.withRetry('Poll transcription status', async () =>
          axios.get(`${this.baseUrl}/transcript/${transcriptId}`, {
            headers: {
              Authorization: this.apiKey,
            },
          }),
        ...
        this.logger.debug(
          `[AssemblyAI] Transcription status: ${status} (attempt ${attempt + 1}/${maxAttempts})`,
        );
  ...
  async getSrtSubtitles(transcriptId: string): Promise<string> {
    ...
    try {
      const response = await axios.get(`${this.baseUrl}/transcript/${transcriptId}/srt`, {
        headers: {
          Authorization: this.apiKey,
        },
      });
      ...
      this.logger.log(
        `[AssemblyAI] SRT subtitles fetched successfully (${response.data.length} bytes)`,
      );
      return response.data;
    } catch (error) {
  ...
  async transcribe(
    audioPath: string,
    languageCode?: string | null,
  ): Promise<{ whisperFormat: any; transcriptId: string }> {
