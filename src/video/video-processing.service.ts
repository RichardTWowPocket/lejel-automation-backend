...
export class VideoProcessingService {
  ...
  constructor(
    ...
    @Inject(forwardRef(() => require('../assemblyai/assemblyai.service').AssemblyAIService))
    private assemblyAIService: any,
  ) {
    ...
    try {
      ...
      if (shouldUseWhisper) {
        ...
        try {
          ...
          const { result: transcriptionResult, transcriptId } = await this.transcriptionService.transcribe(
            extractedAudioPath,
          ...
          this.logger.log(`${logPrefix} [VideoProcessingService] Transcript ID: ${transcriptId}`);
          ...
          // Keep subtitle files for debugging (don't delete immediately)
      ...
      for (let i = 0; i < sections.length; i++) {
        ...
        if (shouldAddSubtitles) {
          try {
            ...
            this.logger.log(`[combine-mediaa] Transcription completed (Transcript ID: ${transcriptId})`);
...
`;
  ...
  private convertColorToASS(color: string): string {
    ...
    // If it's already in ASS format (&HBBGGRR& or &HBBGGRR), return the color part
    if (color.startsWith('&H')) {
      // Clean up ASS format: remove &H, trailing &, and any H
      return color.replace(/&H/g, '').replace(/&/g, '').toUpperCase();
    }

    // Standard color names to Hex
    ...
    const namedColor = colorNames[color.toLowerCase()];
  ...
  private async generateHeadlineASS(
    ...
    height: number,
    tempDir: string,
    headlineConfig?: any, // HeadlineConfig from profile
    bottomHeadlineAppear: string = 'start',
    lastSectionStartTime: number = 0,
  ): Promise<string> {
    const assFilePath = path.join(tempDir, `headline_${Date.now()}.ass`);
    // Extract config values with defaults
    const topConfig = headlineConfig?.topHeadline || {
    ...
    const bottomConfig = headlineConfig?.bottomHeadline || {
      fontFamily: 'Hakgyoansim Jiugae',
      fontSize: 100,
      color: 'white',
      highlightColorASS: '&H0000FF&',
    ...
    const topPrimaryColor = this.convertColorToASS(topConfig.color);
    const topHighlightColor = this.convertColorToASS(topConfig.highlightColorASS);
    const bottomPrimaryColor = this.convertColorToASS(bottomConfig.color);
    const bottomHighlightColor = this.convertColorToASS(bottomConfig.highlightColorASS);
...
[V4+ Styles]
Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
Style: TopWhite,${topConfig.fontFamily},${topConfig.fontSize},&H00${topPrimaryColor},&H00${topPrimaryColor},&H00000000,&H80000000,${topConfig.bold ? '-1' : '0'},${topConfig.italic ? '-1' : '0'},0,0,100,100,0,0,1,${topConfig.borderWidth},0,${topConfig.alignment},${topConfig.marginL},${topConfig.marginR},${topConfig.marginV},1
Style: TopHighlight,${topConfig.fontFamily},${topConfig.fontSize},&H00${topHighlightColor},&H00${topHighlightColor},&H00000000,&H80000000,${topConfig.bold ? '-1' : '0'},${topConfig.italic ? '-1' : '0'},0,0,100,100,0,0,1,${topConfig.borderWidth},0,${topConfig.alignment},${topConfig.marginL},${topConfig.marginR},${topConfig.marginV},1
Style: BottomWhite,${bottomConfig.fontFamily},${bottomConfig.fontSize},&H00${bottomPrimaryColor},&H00${bottomPrimaryColor},&H00000000,&H80000000,${bottomConfig.bold ? '-1' : '0'},${bottomConfig.italic ? '-1' : '0'},0,0,100,100,0,0,1,${bottomConfig.borderWidth},0,${bottomConfig.alignment},${bottomConfig.marginL},${bottomConfig.marginR},${bottomConfig.marginV},1
Style: BottomHighlight,${bottomConfig.fontFamily},${bottomConfig.fontSize},&H00${bottomHighlightColor},&H00${bottomHighlightColor},&H00000000,&H80000000,${bottomConfig.bold ? '-1' : '0'},${bottomConfig.italic ? '-1' : '0'},0,0,100,100,0,0,1,${bottomConfig.borderWidth},0,${bottomConfig.alignment},${bottomConfig.marginL},${bottomConfig.marginR},${bottomConfig.marginV},1
...
`;
    ...
    // Returns inline style overrides for highlights using config color
    const parseTextWithHighlight = (text: string, highlightColor: string, primaryColor: string): string => {
      // Normalize spaces
      ...
      while ((match = regex.exec(normalized)) !== null) {
        ...
        // Add highlighted text with color override from config
        if (match[1]) {
          result += `{\\c&H${highlightColor}}${match[1]}{\\c&H${primaryColor}}`;
        }
    ...
    if (topHeadlineText) {
      ...
      const textLines = lines.map(line => parseTextWithHighlight(line, topHighlightColor, topPrimaryColor));

      // Simple lineHeight implementation for multi-line headlines
      // If lineHeight is specified, we can add small vertical shifts to lines
      const lineHeightValue = topConfig.lineHeight || 1.0;
      let fullText = '';

      if (lineHeightValue !== 1.0 && textLines.length > 1) {
        // Calculate line relative vertical offset using font size and line height
        // In ASS, \pos(x,y) is better for exact line positioning, but \N is simpler.
        // We can use {\fs<size>} trick or just {\fsp<spacing>} for some control, 
        // but for TRUE line height between lines in one dialogue, ASS is limited without complex tags.
        // Let's use negative \fscy or just accept it's hard.
        // Better: use {\vsp<vertical spacing>} tag if supported by renderer, but it's not standard.
        // Standard way is to create multiple dialogue lines with different margins.
        fullText = textLines.join('\\N');
      } else {
        fullText = textLines.join('\\N');
      }
    ...
    if (bottomHeadlineText) {
      ...
      // Build multi-line text with \\N separators
      const textLines = lines.map(line => parseTextWithHighlight(line, bottomHighlightColor, bottomPrimaryColor));

      // Implement lineHeight if provided (ASS doesn't have a direct lineHeight, so we use Spacing or multiple styles)
      // A common trick is to use \fsp for character spacing or just multiple lines.
      // But for vertical spacing between lines, we can use \q2 (no wrap) and add actual vertical gaps.
      // For now, we'll just handle it as a single block.
      const fullText = textLines.join('\\N');

      // Dialogue entry: check if it should only appear at the end
      const bottomStartTime = bottomHeadlineAppear === 'last' ? formatTime(lastSectionStartTime) : startTime;
      assContent += `Dialogue: 0,${bottomStartTime},${endTime},BottomWhite,,0,0,0,,${fullText}\n`;
    }
...
`;
  ...
  async addSubtitlesToVideo(
    ...
    // For logging/debugging: Detect dialogue entries if it's an ASS file
    const dialogueCount = (subtitleFileContent.match(/^Dialogue:/gm) || []).length;
    this.logger.debug(`[addSubtitlesToVideo] Detected ${dialogueCount} dialogue entries in subtitle file`);
    ...
    this.logger.log(`[addSubtitlesToVideo] Using ASS filter for subtitles (isSocialMediaStyle=${isSocialMediaStyle})`);

    // Use single quotes around the path to handle spaces and special chars for the ass filter itself
    // However, fluent-ffmpeg's .videoFilters() will handle the escaping for the shell
    const filterString = `ass=${escapedSubtitlePath}`;
    ...
    return new Promise((resolve, reject) => {
      ...
      ffmpegCommand
        .on('start', (commandLine) => {
          ...
          const ffmpegCommandPath = path.join(path.dirname(absoluteSubtitlePath), `ffmpeg_command_${Date.now()}.txt`);
          fs.writeFileSync(ffmpegCommandPath, commandLine, 'utf8');
          this.logger.log(`[addSubtitlesToVideo] ✅ Saved FFmpeg command to debug file: ${ffmpegCommandPath}`);
          this.scheduleFileCleanup(ffmpegCommandPath);
        })
        .on('progress', (progress) => {
          if (progress.percent) {
   ...
   * @param sections - Array of sections with transcript and image path
   * @param outputFormat - Output video format (default: 'mp4')
   * @param requestId - Request ID for logging
   * @param profileConfig - Full profile configuration (optional, uses defaults if not provided)
   * @param topHeadlineText - Override top headline text from request
   * @param bottomHeadlineText - Override bottom headline text from request
   * @param bottomHeadlineAppear - When to show bottom headline ('start' or 'last')
   */
  async combineMediasWithTranscripts(
    ...
    outputFormat: string = 'mp4',
    requestId?: string,
    profileConfig?: any, // ProfileConfig type (using any for now to avoid circular dependency)
    ...
    // Extract config values from profile with defaults
    const subtitleConfig = profileConfig?.subtitle || {
      useSubtitle: false,
      useSocialMediaSubtitle: false,
    ...
    const useSubtitle = subtitleConfig.useSubtitle;
    const useSocialMediaSubtitle = subtitleConfig.useSocialMediaSubtitle;
    const layout = layoutConfig.type;
    const width = layoutConfig.canvasWidth;
    const height = layoutConfig.canvasHeight;
    const verticalGap = layoutConfig.verticalGap;
    const imageAspect = layoutConfig.imageAspect;
    const inputRatio = layoutConfig.inputRatio;
    ...
    try {
      ...
      if (useSubtitle || useSocialMediaSubtitle) {
        ...
        const { result, transcriptId } = await this.transcriptionService.transcribe(
          resolvedAudioPath,
          undefined, // Auto-detect language
          responseFormat,
        );
        ...
        // Step 2.5: Calculate section timings from whisper result
        // This is needed for both image durations and subtitle synchronization
        this.logger.debug(`${logPrefix} [combineMediasWithTranscripts] Step 2.5: Calculating section timings...`);
        ...
        if (useSubtitle || useSocialMediaSubtitle) {
          this.logger.log(`${logPrefix} [combineMediasWithTranscripts] Generating subtitles (style: ${useSocialMediaSubtitle ? 'Social Media / Karaoke' : 'Plain'})...`);
      ...
      if (layout === 'vertical_poster' && (topHeadlineText || bottomHeadlineText)) {
        ...
        const lastSectionTiming = sectionTimings[sectionTimings.length - 1];
        const lastSectionStartTime = lastSectionTiming ? lastSectionTiming.start : 0;
        ...
        const headlineAssPath = await this.generateHeadlineASS(
          ...
          finalHeight,
          absoluteTempDir,
          headlineConfig, // Pass headline config from profile
          bottomHeadlineAppear || 'start',
          lastSectionStartTime,
        );
  ...
  private async generateSRTFromTranscripts(
    ...
    await fsPromises.writeFile(srtFilePath, srtContent, 'utf8');
...
`;
  ...
  private async generateASSFromTranscripts(
    ...
    tempDir: string,
    logPrefix: string,
    useKaraokeStyle: boolean = true,
    subtitleConfig?: any,
    audioDuration: number = 0,
    sectionTimings: Array<{ start: number; end: number; duration: number }> = [],
    ...
    // Standardize color format to 8-digit AABBGGRR
    const primaryColorASS = subtitleConfig?.primaryColor ? ('&H' + this.convertColorToASS(subtitleConfig.primaryColor).replace(/&H|&/g, '').padStart(8, '0')) : (useKaraokeStyle ? '&H0000FFFF' : '&H00FFFFFF');
    const secondaryColorASS = subtitleConfig?.secondaryColor ? ('&H' + this.convertColorToASS(subtitleConfig.secondaryColor).replace(/&H|&/g, '').padStart(8, '0')) : '&H00FFFFFF';
    const outlineColorASS = subtitleConfig?.outlineColor ? ('&H' + this.convertColorToASS(subtitleConfig.outlineColor).replace(/&H|&/g, '').padStart(8, '0')) : '&H00000000';
    const backColorASS = subtitleConfig?.backColor ? ('&H' + this.convertColorToASS(subtitleConfig.backColor).replace(/&H|&/g, '').padStart(8, '0')) : '&H80000000';

    const bold = subtitleConfig?.bold === true ? '-1' : '0';
    const italic = subtitleConfig?.italic === true ? '-1' : '0';
    const borderStyle = subtitleConfig?.borderStyle || '1';
    const outlineWidth = subtitleConfig?.outlineWidth || subtitleConfig?.outline || 2;
    const shadow = subtitleConfig?.shadow || 0;
    const alignment = subtitleConfig?.alignment || 2;

    // MarginV: If it's the default 960 (middle), it might overlap with images.
    // For regular subtitles, we often want them lower.
    const marginV = subtitleConfig?.marginV || 100;
    const marginL = subtitleConfig?.marginL || 10;
    const marginR = subtitleConfig?.marginR || 10;

    const title = useKaraokeStyle ? 'Social Media Subtitles' : 'Plain Subtitles';
    // Use PlayResX and PlayResY for consistent font scaling
    let assContent = `[Script Info]\nTitle: ${title}\nScriptType: v4.00+\nPlayResX: ${canvasWidth}\nPlayResY: ${canvasHeight}\n\n[V4+ Styles]\nFormat: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding\nStyle: Default,${fontFamily},${fontSize},${primaryColorASS},${secondaryColorASS},${outlineColorASS},${backColorASS},${bold},${italic},0,0,100,100,0,0,${borderStyle},${outlineWidth},${shadow},${alignment},${marginL},${marginR},${marginV},1\n\n[Events]\nFormat: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text\n`;
    ...
    for (let i = 0; i < transcripts.length; i++) {
      const userWords = transcripts[i].trim().split(/\s+/).filter(w => w.length > 0);
      if (userWords.length === 0) continue;

      const timing = (sectionTimings && sectionTimings[i]) || {
        start: i * (audioDuration / transcripts.length),
        end: (i + 1) * (audioDuration / transcripts.length)
      };

      const sectionStartTime = timing.start;
      const sectionEndTime = timing.end;
      const sectionDuration = sectionEndTime - sectionStartTime;

      // Process into chunks for both plain and karaoke styles
      // This ensures subtitles are always synchronized with the audio
      const wordsWithTiming: Array<{ text: string; start: number; end: number }> = [];
      for (let wordIdx = 0; wordIdx < userWords.length; wordIdx++) {
        const dur = sectionDuration / (userWords.length || 1);
        wordsWithTiming.push({
          text: userWords[wordIdx],
          start: sectionStartTime + (wordIdx * dur),
          end: sectionStartTime + ((wordIdx + 1) * dur),
        });
      }

      // Chunk size: 3 for karaoke (fast highlighting), 5 for plain (readable blocks)
      const chunkSize = useKaraokeStyle ? 3 : 5;

      for (let j = 0; j < wordsWithTiming.length; j += chunkSize) {
        const chunk = wordsWithTiming.slice(j, j + chunkSize);
        if (chunk.length === 0) continue;
        ...
        if (useKaraokeStyle) {
          for (let k = 0; k < chunk.length; k++) {
            const word = chunk[k];
            // Use centiseconds for \k tag (rounded to nearest integer, at least 1)
            const durCs = Math.max(1, Math.round((word.end - word.start) * 100));
            text += `{\\k${durCs}}${this.escapeTextForAss(word.text)}${k < chunk.length - 1 ? ' ' : ''}`;
          }
        } else {
          // Plain mode: skip the \k tags, just join text
          text = chunk.map(w => this.escapeTextForAss(w.text)).join(' ');
        }

        assContent += `Dialogue: 0,${this.formatAssTime(chunkStart)},${this.formatAssTime(chunkEnd)},Default,,0,0,0,,${text}\n`;
      }
    }

    const assFileName = `subtitles_${useKaraokeStyle ? 'karaoke' : 'plain'}_${Date.now()}.ass`;
    const assFilePath = path.join(tempDir, assFileName);

    // SAVE DEBUG LOG
    this.logger.log(`${logPrefix} [generateASSFromTranscripts] Final ASS file path: ${assFilePath}`);
    this.logger.debug(`${logPrefix} [generateASSFromTranscripts] ASS Styling: Font=${fontFamily}, Size=${fontSize}, MarginV=${marginV}, Primary=${primaryColorASS}`);

    await fsPromises.writeFile(assFilePath, assContent, 'utf8');
    this.scheduleFileCleanup(assFilePath);
    return assFilePath;
  }

  /**
   * Calculate section timings from Whisper result with robust fallback
   */
  private async calculateSectionTimingsFromWhisper(
    ...
    this.logger.log(`${logPrefix} [calculateSectionTimingsFromWhisper] Deterministic Anchor Matching for ${transcripts.length} sections...`);
    ...
    if (allWords.length === 0) {
      this.logger.warn(`${logPrefix} [calculateSectionTimingsFromWhisper] No words found in transcription, using simple division`);
      const dur = audioDuration / transcripts.length;
      return transcripts.map((_, i) => ({ start: i * dur, end: (i + 1) * dur, duration: dur }));
    }

    // Step 1: Find the "Anchor Index" (the index in allWords where each section starts)
    const anchorIndices: number[] = new Array(transcripts.length).fill(-1);
    let searchStartIndex = 0;

    for (let i = 0; i < transcripts.length; i++) {
      const rawUserWords = transcripts[i].trim().split(/\s+/).filter(w => w.length > 0);
      if (rawUserWords.length === 0) continue;

      const userWords = rawUserWords.map(w => this.normalizeWord(w));

      // Try to find the first word, or second, or third... (robustness against missing words)
      for (let wordOffset = 0; wordOffset < Math.min(3, userWords.length); wordOffset++) {
        const targetWord = userWords[wordOffset];
        const searchLimit = Math.min(allWords.length, searchStartIndex + 100); // 100 word window

        let matchIndex = -1;
        for (let j = searchStartIndex; j < searchLimit; j++) {
          if (this.wordsMatch(targetWord, this.normalizeWord(allWords[j].word))) {
            matchIndex = j;
            break;
          }
        }
    ...
    // Step 2: Build the raw timeline using the look-ahead logic
    const rawTimings: Array<{ start: number; end: number; duration: number }> = [];
    ...
    for (let i = 0; i < transcripts.length; i++) {
      ...
      if (i === 0) {
        ...
        start = 0;
      } else if (anchorIndices[i] !== -1) {
        start = allWords[anchorIndices[i]].start;
      } else {
        // If no anchor found, use previous end (continuity)
        start = rawTimings[i - 1].end;
      }

      // End time (Look-ahead):
      if (i === transcripts.length - 1) {
        // Last section ends at audio duration
        end = audioDuration;
      } else {
        // Look for the next available anchor
        let nextAnchorIdx = -1;
        for (let j = i + 1; j < transcripts.length; j++) {
          if (anchorIndices[j] !== -1) {
            nextAnchorIdx = anchorIndices[j];
            break;
          }
        }

        if (nextAnchorIdx !== -1) {
          // Section ends at the word immediately preceding the next anchor
          end = allWords[Math.max(0, nextAnchorIdx - 1)].end;
        } else {
          // No future anchors found, distribute remaining time
          const remainingSections = transcripts.length - i;
          const timeRemaining = audioDuration - start;
          end = start + (timeRemaining / remainingSections);
        }
      }

      // Safety: ensure strictly increasing
      const prevEnd = i > 0 ? rawTimings[i - 1].end : 0;
      if (start < prevEnd) start = prevEnd;
      if (end <= start) end = start + 1.0;

      rawTimings.push({ start, end, duration: end - start });
    }

    // Step 3: FRAME SYNC PASS (30 FPS)
    const sectionTimings: Array<{ start: number; end: number; duration: number }> = [];
    let currentTime = 0;

    for (let i = 0; i < rawTimings.length; i++) {
      const rawDur = rawTimings[i].end - rawTimings[i].start;
      const frameAlignedDur = Math.round(rawDur * FPS) / FPS;
      const start = currentTime;
      const end = start + frameAlignedDur;

      sectionTimings.push({
        start,
        end: i === rawTimings.length - 1 ? audioDuration : end,
        duration: i === rawTimings.length - 1 ? (Math.max(0.1, audioDuration - start)) : frameAlignedDur
      });

      currentTime = end;
    }

    this.logger.log(`${logPrefix} [calculateSectionTimingsFromWhisper] ✅ Deterministic timing complete. Final length: ${sectionTimings[sectionTimings.length - 1]?.end.toFixed(2)}s`);
  ...
  private normalizeWord(word: string): string {
    if (!word) return '';
    // Remove common punctuation and spaces, including Korean ones
  ...
  /**
   * Convert SRT subtitles from AssemblyAI to ASS format
   */
  private async convertSrtToAss(
    srtContent: string,
    tempDir: string,
    subtitleConfig: any,
  ): Promise<string> {
    this.logger.log(`[convertSrtToAss] Converting SRT to ASS format...`);

    // Parse SRT entries
    const srtEntries = this.parseSrt(srtContent);
    this.logger.log(`[convertSrtToAss] Parsed ${srtEntries.length} SRT entries`);

    // Generate ASS header with subtitle config
    const fontFamily = subtitleConfig?.fontFamily || 'Noto Sans CJK KR';
    const fontSize = subtitleConfig?.fontSize || 72; // Larger font for better visibility
    const marginV = subtitleConfig?.marginV || 200;
    const primaryColor = subtitleConfig?.primaryColor || '&H00FFFFFF'; // White
    const outlineColor = subtitleConfig?.outlineColor || '&H00000000'; // Black
    const outlineWidth = subtitleConfig?.outlineWidth || 8; // Thick outline for visibility
...
[V4+ Styles]
Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding
Style: Default,${fontFamily},${fontSize},${primaryColor},${primaryColor},${outlineColor},&H80000000,-1,0,0,0,100,100,0,0,1,${outlineWidth},2,2,50,50,${marginV},1

[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
`;

    // Convert each SRT entry to ASS dialogue
    for (const entry of srtEntries) {
      const text = entry.text.replace(/\n/g, '\\N'); // ASS uses \N for line breaks
      const escapedText = this.escapeTextForAss(text);
      assContent += `Dialogue: 0,${entry.start},${entry.end},Default,,0,0,0,,${escapedText}\n`;
    }

    const assFilePath = path.join(tempDir, `assemblyai_srt_${Date.now()}.ass`);
    await fsPromises.writeFile(assFilePath, assContent, 'utf8');

    this.logger.log(`[convertSrtToAss] ASS file created: ${assFilePath} (${srtEntries.length} entries)`);
    return assFilePath;
  }
   ...
   */
  private parseSrt(srtContent: string): Array<{ start: string; end: string; text: string }> {
    const entries = [];
    const blocks = srtContent.trim().split('\n\n');
    ...
    for (const block of blocks) {
      ...
      const startSrt = match[1];
      const endSrt = match[2];
      const text = lines.slice(2).join('\n');

      // Convert SRT time to ASS time (00:00:13,160 -> 0:00:13.16)
      const startAss = this.srtTimeToAssTime(startSrt);
      const endAss = this.srtTimeToAssTime(endSrt);

      entries.push({ start: startAss, end: endAss, text });
    }
   ...
   */
  private srtTimeToAssTime(srtTime: string): string {
    // Convert 00:00:13,160 to 0:00:13.16
    return srtTime.replace(',', '.').replace(/^00:/, '0:');
  }
}
