...
import { SegmentScriptDto, SegmentScriptSource } from './dto/segment-script.dto';
...
export class LlmController {
  ...
  async segmentScript(@Body() dto: SegmentScriptDto) {
    ...
    const scriptSource: SegmentScriptSource = dto.scriptSource ?? 'manual';
    const { segments, fullScript } = await this.llmService.segmentScriptForDashboard(
      dto.fullScript,
      model,
      {
        scriptSource,
        articleTitle: dto.articleTitle?.trim() || null,
